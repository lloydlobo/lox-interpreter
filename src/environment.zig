...
pub fn getAuto(self: *const Self, comptime K: type, name: K) Error!Value {
    switch (@TypeOf(name)) {
        []const u8, []u8 => {
            assert(name.len > 0 and root.isAlphaNumeric(name[0]));

            if (self.values.get(name)) |value| return value;
            if (self.enclosing) |enclosing| return try enclosing.getAuto(K, name);

            return Error.variable_not_declared;
        },
        Token => return try self.get(name),
...
/// Example:
/// ```zig
/// return try self.closure.getAt_([]const u8, 0, "this");
/// ```
pub fn getAtAuto(self: *Environment, comptime K: type, distance: i32, name: K) Error!Value {
    return try self.ancestor(distance).getAuto(K, name);
}
